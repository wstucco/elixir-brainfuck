defmodule BrainfuckTest do
  ...
  test "BF program: Hello World!" do
    {_addr, mem, _input, output} = Brainfuck.run load_fixture "hello_world.bf"
  ...
  test "BF program: modified Hello World!" do
    {_addr, mem, _input, output} = Brainfuck.run load_fixture "hello_world_mod.bf"
  ...
  test "BF program: copy input to output" do
    ...
    assert mem == [0]
    assert mem |> length == 1
    assert output == "123ABC"
  end

  test "BF program: reverse input" do
    {_addr, mem, _input, output} = Brainfuck.run(">,[>,]<[.<]", "123ABC")
    assert mem == [0, 49, 50, 51, 65, 66, 67, 0]
    assert mem |> length == 8
